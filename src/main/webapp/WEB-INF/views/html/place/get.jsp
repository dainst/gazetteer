				...
				<%@ page contentType="text/html; charset=utf-8" session="false" %>
					...
					<head>
						<title>
							<c:choose>
								<c:when
									test="${accessStatus == 'READ' || accessStatus == 'LIMITED_READ' || accessStatus == 'EDIT'}">
						...
						<link href="//netdna.bootstrapcdn.com/font-awesome/3.0.2/css/font-awesome-ie7.css"
							rel="stylesheet">
					...
					<body>
						...
						<div class="archaeo-fixed-menu">
							...
							<div class="affix-menu-wrapper">
								<div id="affix-menu" style="z-index: 100000" class="navbar navbar-inverse container"
									data-spy="affix">
									<div class="navbar-inner">
										...
										<div class="nav-collapse pull-left">
											<ul class="nav">
												<li><a href="app/#!/thesaurus">
														<s:message code="ui.thesaurus.list" text="ui.thesaurus.list" />
													</a></li>
												<li><a href="app/#!/extended-search">
														<s:message code="ui.search.extendedSearch"
															text="ui.search.extendedSearch" />
													</a></li>
												<li><a href="app/#!/create/">
						...
						<div class="container" itemscope itemtype="http://schema.org/Place">
							...
							<div class="page-header">
								<h2>
									<c:choose>
										...
										<c:otherwise>
											<s:message code="domain.place.hiddenPlace"
												text="domain.place.hiddenPlace" />
										</c:otherwise>
									</c:choose>

									<small><a href="${baseUri}place/${place.id}"
											itemprop="url">${baseUri}place/${place.id}</a> <a data-toggle="modal"
											href="#copyUriModal"><i class="icon-share"
												style="font-size:0.7em"></i></a></small>
								</h2>
							</div>
							...
							<div class="modal hide" id="copyUriModal">
								...
								<div class="modal-body">
									<label>${copyMsg}</label>
									<input class="input-xxlarge" style="width:97%" type="text"
										value="${baseUri}place/${place.id}" id="copyUriInput"></input>
								</div>
							</div>
							<script type="text/javascript">
							...
							<h2>
								<s:message code="ui.otherFormats" text="ui.otherFormats" />
							</h2>
							<ul>
							...
							<c:if test="${accessStatus == 'READ' || accessStatus == 'EDIT'}">
								...
								<!-- names -->
								<h2>
									<s:message code="domain.place.names" />
								</h2>
								<ul>
									<li>
										<em>
								...
								<c:if test="${parent != null}">
									...
									<p>
										<a href="${baseUri}place/${parent.id}"
											itemprop="containedIn">${parent.prefName.title}
											<c:if test="${parent.types != null && !empty(parent.types)}">
												<em>(
													<s:message code="place.types.${parent.types.toArray()[0]}"
														text="${parent.types.toArray()[0]}" />)
												</em>
											</c:if>
										</a>
									</p>
								...
								<c:if test="${!empty(children)}">
									<h2>
										<s:message code="domain.place.children" text="domain.place.children" />
									</h2>
									<p>
										<a
											href="${baseUri}search.kml?q=parent:${place.id}&limit=${fn:length(children)}">
											<s:message code="ui.numberOfPlaces" text="ui.numberOfPlaces"
												arguments="${fn:length(children)}" />
										</a>
									</p>
								</c:if>
								...
								<c:if test="${!empty(relatedPlaces)}">
									<h2>
										<s:message code="domain.place.relatedPlaces"
											text="domain.place.relatedPlaces" />
									</h2>
									<ul>
										<c:forEach var="relatedPlace" items="${relatedPlaces}">
											<li>
												<a href="${baseUri}place/${relatedPlace.id}">${relatedPlace.prefName.title}
													<c:if
														test="${relatedPlace.types != null && !empty(relatedPlace.types)}">
														<em>(
															<s:message
																code="place.types.${relatedPlace.types.toArray()[0]}"
																text="${relatedPlace.types.toArray()[0]}" />)
														</em>
													</c:if>
												</a>
											</li>
								...
								<c:if test="${place.prefLocation != null}">
									...
									<ul>
										<li itemprop="geo" itemscope itemtype="http://schema.org/GeoCoordinates">
											<em>
												<s:message code="domain.location.latitude"
													text="domain.location.latitude" />:
											</em> ${place.prefLocation.lat},
											<em>
												<s:message code="domain.location.longitude"
													text="domain.location.longitude" />:
											</em> ${place.prefLocation.lng}
											(<em>
												<s:message code="domain.location.confidence"
													text="domain.location.confidence" />:
											</em>
											<s:message code="confidence.${place.prefLocation.confidence}"
												text="${place.prefLocation.confidence}" />)
										...
										<c:forEach var="location" items="${place.locations}">
											<li>
												...
												<s:message code="confidence.${location.confidence}"
								...
								<c:if test="${not empty place.arachneId or not empty place.zenonId}">
									<h2>
										<s:message code="ui.contexts" text="ui.contexts" />
									</h2>
									<ul>
										<c:if test="${not empty place.arachneId}">
											<li>
												<a href="http://arachne.uni-koeln.de/arachne/index.php?view[layout]=search_result_overview&view[category]=overview&search[constraints]=FS_OrtID:%22${place.arachneId}%22"
													target="_blank">
										...
										<c:if test="${not empty place.zenonId}">
											<li>
												<a href="http://zenon.dainst.org/#search?q=f999_1:${place.zenonId}"
													target="_blank">
								...
								<c:if test="${!empty(place.identifiers)}">
									<h2>
										<s:message code="domain.place.identifiers" text="domain.place.identifiers" />
									</h2>
									<ul>
								...
								<c:if test="${!empty(place.links)}">
									...
									<ul>
										<c:forEach var="link" items="${place.links}">
											<li>
												<em>${link.predicate}:</em> <a href="${link.object}"
													target="_blank">${link.object}</a>
											</li>
										</c:forEach>
									</ul>
								</c:if>
