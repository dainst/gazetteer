var messages = angular.module('gazetteer.messages', []);

messages.factory('messages', function(){
	return {
		"ui.error": "Fehler",
		"ui.contactAdmin": "Wenden Sie sich bitte an arachne@uni-koeln.de, falls das Problem länger besteht.",
		"ui.search.results": "Suchergebnis",
		"ui.search.hits": "Treffer",
		"ui.search.limit.10.tooltip": "10 Treffer pro Seite zeigen",
		"ui.search.limit.50.tooltip": "50 Treffer pro Seite zeigen",
		"ui.search.limit.100.tooltip": "100 Treffer pro Seite zeigen",
		"ui.search.limit.1000.tooltip": "1000 Treffer pro Seite zeigen",
		"ui.search.sort.score.tooltip": "Nach Relevanz sortieren",
		"ui.search.sort.id.tooltip": "Nach ID sortieren",
		"ui.search.sort.name.tooltip": "Nach Namen sortieren",
		"ui.search.sort.type.tooltip": "Nach Art sortieren",
		"ui.search.sort.thesaurus.tooltip": "Nach Thesaurus sortieren",
		"ui.search.filter": "Filter",
		"ui.search.filter.coordinates": "Mit Koordinaten",
		"ui.search.filter.no-coordinates": "Ohne Koordinaten",
		"ui.search.filter.polygon": "Mit Polygon",
		"ui.search.filter.no-polygon": "Ohne Polygon",
		"ui.search.filter.unlocatable": "Nicht lokalisierbar",
		"ui.search.filter.no-tags": "Ohne Tags",
		"ui.search.filter.no-provenance": "Ohne Provenienzangabe",
		"ui.search.grandchildren-search": "Alle Unterorte",
		"ui.search.result-limit-exceeded": "Die Suchmenge überschreitet 10000 Einträge. Bitte schränken Sie die Suche weiter ein.",
		"ui.place.names.more": "mehr",
		"ui.place.names.less": "weniger",
		"ui.place.children.search": "Orte in Suche anzeigen",
		"ui.place.save.success": "Ort erfolgreich gespeichert",
		"ui.place.save.failure": "Ort konnte nicht gespeichert werden",
		"ui.place.save.failure.parentError": "Ein Ort darf nicht sein eigener untergeordneter Ort oder derjenige eines Ortes sein, den er enthält.",
		"ui.place.save.failure.accessDeniedError": "Sie verfügen nicht über die nötige Berechtigung, um diesen Ort zu bearbeiten.",
		"ui.place.duplicate.success": "Ort erfolgreich dupliziert",
		"ui.place.duplicate.failure": "Ort konnte nicht dupliziert werden",
		"ui.place.remove.success": "Ort erfolgreich gelöscht",
		"ui.place.remove.failure": "Ort konnte nicht gelöscht werden",
		"ui.place.protected-site-login-info": "Bitte loggen Sie sich ein, um Zugriff auf die exakten Koordinaten zu erhalten.",
		"ui.place.protected-site-group-info": "Sie verfügen nicht über die nötigen Gruppenberechtigungen, um auf die exakten Koordinaten zuzugreifen. Bitte kontaktieren Sie die Administratoren der Datensatzgruppe.",
		"ui.place.user-group-info": "Wenn eine Datensatzgruppe festgelegt wird, ist der Ort nur für Mitglieder dieser Gruppe sichtbar. Diese Einstellung kann zu einem späteren Zeitpunkt nicht mehr geändert werden.",
		"ui.place.tags.info": "Wenn möglich bitte bereits vorhandene Tags nutzen. Für neue Tags bitte stets englische Begriffe, kleingeschrieben im Singular verwenden: \"castle\"",
		"ui.place.replacing.first": "Der Ort mit der ID ",
		"ui.place.replacing.second": " wurde ersetzt. Sie wurden zum neuen Ortseintrag weitergeleitet.",
		"ui.recordGroupContact.success": "Ihre Nachricht wurde erfolgreich versendet.",
		"ui.recordGroupContact.error": "Beim Versenden Ihrer Nachricht ist ein Fehler aufgetreten.",
		"ui.thesaurus": "Thesaurus",
		"ui.create": "Ort anlegen",
		"ui.link.tooltip": "Mit aktuellem Ort verknüpfen",
		"ui.place.deleted": "Dieser Ort wurde gelöscht",
		"ui.place.hiddenPlace": "Versteckter Ort",
		"ui.place.provenance-info": "Dieses Objekt enthält Daten aus den angegebenen Quellen.",
		"ui.merge.tooltip": "Diesen und aktuellen Ort zusammenfassen",
		"ui.merge.success.head": "Zusammenfassen erfolgreich",
		"ui.merge.success.body": "Bitte überprüfen Sie unten die Angaben des neu angelegten Orts",
		"ui.extendedSearch": "Erweiterte Suche",
		"ui.change-history.change-type.create": "Erstellt",
		"ui.change-history.change-type.edit": "Editiert",
		"ui.change-history.change-type.delete": "Gelöscht",
		"ui.change-history.change-type.merge": "Zusammengeführt",
		"ui.change-history.change-type.merge-with": "Zusammengeführt mit",
		"ui.change-history.change-type.replace": "Ersetzt",
		"ui.change-history.change-type.replace-with": "Ersetzt durch",
		"ui.change-history.change-type.duplicate": "Dupliziert",
		"ui.change-history.change-type.unknown": "Unbekannt",
		"ui.about": "Über den Gazetteer",
		"ui.help": "Hilfe",
		"ui.help.editor.success": "Der Text wurde erfolgreich gespeichert.",
		"ui.help.editor.restored": "Der Text wurde erfolgreich wiederhergestellt.",
		"ui.shape-editor.error.wkt.invalidgeometrytype": "Ungültiger Geometrietyp. Erlaubte Geometrietypen sind 'POLYGON' und 'MULTIPOLYGON'.",
		"ui.shape-editor.error.wkt.missingbracket": "Eine schließende Klammer fehlt.",
		"ui.shape-editor.error.wkt.notanumber.1": "Die Koordinate ",
		"ui.shape-editor.error.wkt.notanumber.2": " ist keine gültige Zahl.",
		"ui.shape-editor.error.wkt.genericparsingerror": "WKT-Daten konnten nicht eingelesen werden.",
		"ui.shape-editor.error.geojson.invalidjson": "JSON-Daten konnten nicht eingelesen werden. Der Eingabetext ist kein gültiger JSON-String.",
		"ui.shape-editor.error.geojson.invalidtype": "Ungültiger Typ. Das Feld 'type' des GeoJSON-Objekts muss den Wert 'Feature' enthalten.",
		"ui.shape-editor.error.geojson.nogeometry": "Kein Geometrie-Objekt des Typs 'Polygon' oder 'MultiPolygon' konnte gefunden werden.",
		"ui.shape-editor.error.geojson.invalidgeometrytype.1": "'",
		"ui.shape-editor.error.geojson.invalidgeometrytype.2": "' ist kein unterstützter Geometrietyp. Erlaubte Geometrietypen sind 'Polygon' und 'MultiPolygon'.",
		"ui.shape-editor.error.geojson.emptycoordinates": "Das Geometrie-Objekt enthält keine Koordinaten.",
		"ui.shape-editor.error.validation.pathnotclosed.1": "Der mit dem Punkt (",
		"ui.shape-editor.error.validation.pathnotclosed.2": ") beginnende Pfad ist nicht geschlossen. Der erste und der letzte Punkt jedes Pfades müssen identisch sein.",
		"ui.shape-editor.error.validation.duplicatepoint.1": "Doppelter Punkt (",
		"ui.shape-editor.error.validation.duplicatepoint.2": "). Bitte stellen Sie sicher, dass keine doppelten Punkte außer dem jeweils ersten und letzten Punkt eines Pfades existieren.",
		"ui.shape-editor.error.validation.lineintersection.1": "Schnittpunkt bei ",
		"ui.shape-editor.error.validation.lineintersection.2": ". Bitte stellen Sie sicher, dass die Geometrie keine einander schneidenden Linien enthält.",
		"ui.shape-editor.error.validation.incompletecoordinates": "Unvollständige Koordinaten.",
		"ui.shape-editor.error.validation.genericvalidationerror": "Validierung fehlgeschlagen.",
		"place.name.ancient": "Antik",
		"place.name.transliterated": "Transliteriert",
		"place.types.no-type": " - Keine Art -",
		"place.types.archaeological-site": "Archäologischer Ort",
		"place.types.archaeological-area": "Archäologischer Bereich",
		"place.types.continent": "Kontinent",
		"place.types.administrative-unit": "Verwaltungseinheit",
		"place.types.populated-place": "Bewohnter Ort",
		"place.types.building-institution": "Standort/Institution",
		"place.types.landform": "Landform",
		"place.types.island": "Insel",
		"place.types.hydrography": "Hydrographie",
		"place.types.landcover": "Landbedeckung",
		"place.types.description.archaeological-site": "Ort mit archäologisch relevanten Strukturen (Rom, Akropolis, Grabungsareal)",
		"place.types.description.archaeological-area": "Archäologisch definierter Kulturraum und historische Verwaltungseinheiten (Lydien, Africa proconsularis)",
		"place.types.description.continent": "Durch natürliche oder historische Grenzen getrennte Festlandmassen (Afrika)",
		"place.types.description.administrative-unit": "Politisch administrativ definierte Verwaltungseinheiten (Deutschland, Attika, Metropolitanstadt Bologna)",
		"place.types.description.populated-place": "Von Menschen bewohnter Ort (Rom, Oberammergau)",
		"place.types.description.building-institution": "Standort von Museen und anderen Institutionen (Bodemuseum, DAI Madrid)",
		"place.types.description.landform": "Geomorphologisches Geländemerkmal (Berg, Tal, Höhle)",
		"place.types.description.island": "Vollständig von Wasser umgebene Landmasse (Kreta)",
		"place.types.description.hydrography": "Alle größeren (stehenden wie fließenden) Ansammlungen von Wasser (Atlantik, Chiemsee, Nil)",
		"place.types.description.landcover": "Physische und biologische Bedeckung der Erdoberfläche (Wald, Wüste)",
		"place.types.groups.physical-geographic": "Physiogeographische Einheiten",
		"place.types.groups.human-geographic": "Humangeographische Einheiten",
		"place.types.groups.archaeological": "Archäologisch-kulturhistorische Einheiten",
		"place.types.groups.building": "Sonstiges",
		"place.unlocatable": "Nicht lokalisierbarer Ort",
		"location.confidence.0": "Keine Angabe",
		"location.confidence.1": "Ungenau",
		"location.confidence.2": "Genau",
		"location.confidence.3": "Exakt",
		"location.confidence.4": "Falsch",
		"location.public": "Öffentliche Koordinaten",
		"domain.place.parent": "Liegt in",
		"domain.place.types": "Art",
		"domain.place.tags": "Tags"
	};
});